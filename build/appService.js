...
const appService = (app) => {
    ...
    select * from my_account where login_user = "${req.user_id}"`, (err, rows) => {
      if (err) {
        ...
        rows.map((row, index) => {
          ...
          let dataObj = {}
          row.map((value, idx) => {
            if (dataObj[value.account_name]) {
              dataObj[value.account_name] += value.amount
            }else {
              dataObj[value.account_name] = value.amount
            }
            amount += value.amount
  ...
  app.get('/api/account/list', (req, res) => {
    db.query(`select * from my_account where login_user = "${req.user_id}"`, (err, rows) => {
      if (err) {
        res.json({
          status: 0,
          data: []
  ...
  app.get('/api/income/list', (req, res) => {
    if(!req.query.type){
      db.query(`select * from income where login_user = "${req.user_id}" and type != "normal" and type != "money_management" and date between "${ req.query.start}" and "${req.query.end}"`, (err, rows) => {
        if (err) {
          console.log(err)
          res.json({
            status: 0,
  ...
  app.post('/api/add/income', (req, res) => {
    ...
    (account_id, account_name, account_user, type, login_user, amount, date, description)
     values ("${req.body.account_id}", "${req.body.account_name},"${req.body.account_user}","${req.body.type}","${req.user_id}","${req.amount}","${req.date}","${req.description}") `, (err, rows) => {
      if (err) {
        res.json({
          status: 0
        })
